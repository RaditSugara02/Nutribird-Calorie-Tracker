...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadUserProfileAndEntries() async {
    ...
    setState(() {
      ...
      print(
        'DashboardScreen: Activity entries loaded: ${_activityEntries.length}',
      ); // Debug: Jumlah aktivitas
    });
    ...
    print(
      'DashboardScreen: Raw weight entries JSON from SharedPreferences (before processing): $weightEntriesJson',
    ); // Debug: Tampilkan data mentah yang dimuat
    print(
      'DashboardScreen: Raw weight entries JSON from SharedPreferences (after getStringList): $weightEntriesJson',
    ); // Tambahkan log ini
    setState(() {
      _weightEntries =
          weightEntriesJson
      ...
      print(
        'DashboardScreen: Weight entries loaded into _weightEntries: ${_weightEntries.length}',
      ); // Debug: Jumlah berat badan
    });
  }
  ...
  Future<void> _saveEntries() async {
    ...
    print(
      'DashboardScreen: _weightEntries in _saveEntries before JSON conversion: ${_weightEntries.map((e) => e.weight).toList()}',
    ); // Tambahkan log ini
    final List<String> weightEntriesJson = _weightEntries
        .map((entry) => jsonEncode(entry.toJson()))
        .toList();
    print(
      'Saving weight entries JSON: $weightEntriesJson',
    ); // Debug: Tampilkan data JSON yang akan disimpan
    print(
      'DashboardScreen: Attempting to save weight entries JSON: $weightEntriesJson',
    ); // Tambahkan log ini
    await prefs.setStringList('all_weight_entries', weightEntriesJson);
    print(
      'Weight entries saved to SharedPreferences.',
                          ...
                          children: [
                            Text(
                              DateFormat(
                                'dd MMMM yyyy',
                                'id_ID',
                              ).format(_currentDate), // Langsung format tanggal
                              style: TextStyle(
                                fontSize: 20,
                                fontWeight: FontWeight.bold,
                                color: lightGreenText,
